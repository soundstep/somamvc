package com.soundstep.ui {
	...
	public class ElementUI extends EventDispatcher {
		...
		// keep stage width value to handle fullscreen behavior
		private var _stageWidth:Number = 0;
		// keep stage height value to handle fullscreen behavior
		...
		private function doRefresh():void {
			if (_element.stage != null && _element.stage.displayState == StageDisplayState.FULL_SCREEN && _element.stage.fullScreenSourceRect) {
				// do nothing
			}
			else {
				_stageWidth = _element.stage.stageWidth;
				_stageHeight = _element.stage.stageHeight;
			}
			if (_element is CanvasUI) return;
			var baseWidth:Number = (onStage) ? _stageWidth : _element.parent.width;
			var baseHeight:Number = (onStage) ? _stageHeight : _element.parent.height;
			if (_reference != null) {
				if (_reference is CanvasUI) {
					...
					baseWidth = (_reference == _element.stage) ? _stageWidth : _reference.width;
					baseHeight = (_reference == _element.stage) ? _stageHeight : _reference.height;
				}
			}
		...
		private function removeMask():void {
			_hasMask = false;
			_element.mask = null;
			if (_element.parent != null) _element.parent.removeChild(_bgMask);
		}
