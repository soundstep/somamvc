package com.soma.assets {
    ...
    */
	...
	public class VideoPlayerAsset implements IAsset {
		...
		public function VideoPlayerAsset() {
		...
		/** @private */
		protected function streamStarted(e:SomaVideoEvent):void {
			_player.removeEventListener(SomaVideoEvent.STREAM_STARTED, streamStarted);
			if (_baseUI != null && (_node.hasOwnProperty("@width") && _node.hasOwnProperty("@height"))) SomaUtils.setBaseUIProperties(_player, _baseUI, _node);
		}
				
		/** @private */
		private function originalSizeUpdated(e:SomaVideoEvent):void {
			_player.removeEventListener(SomaVideoEvent.METADATA_UPDATE, originalSizeUpdated);
			SomaUtils.setBaseUIProperties(_player, _baseUI, _node);
			if (_baseUI != null && _baseUI.contains(_player)) {
				var el:ElementUI = _baseUI.getElement(_player);
		...
		public function instantiate(node:XML, baseUI:BaseUI = null):DisplayObject {
			_node = node;
			_baseUI = baseUI;
			_player = new SomaVideoPlayer(_node.@url);
			_player.name = _node.@id;
			_player.addEventListener(SomaVideoEvent.STREAM_STARTED, streamStarted);
			_player.addEventListener(SomaVideoEvent.METADATA_UPDATE, originalSizeUpdated);
			SomaUtils.setProperties(_player, _node, ["url"]);
			return _player;
		}
