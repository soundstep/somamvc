package com.somaprotest.menu.basic {
	...
	public class BasicMenu extends Menu implements IMenu {
		...
		private function clickMenu(m:BasicMenuItem):void {
			...
			deactivateItems(m);
			m.active = true;
			if (m.parent.getChildByName("submenu") == null) openMenuItem(m);
			if (m.type != "" && !autoOpen) {
				new PageEvent(PageEvent.SHOW, m.id).dispatch();
			}
		}
		
		private function openMenuItem(menuItem:BasicMenuItem):void {
			dist = 0;
			...
			function processParent(m:Sprite):void {
				...
				for (var i:uint = 0; i < m.numChildren; i++) {
					if (m.getChildAt(i).name != "submenu") {
						...
						Tweener.addTween(item, {time:.3, y:item.posY});
					}
				}
		...
		private function buildMenu(e:Event = null):void {
			...
			for (var i:int=0; i<arr.length; i++) {
				Tweener.addTween(arr[i], {time:.5, delay:i*.07, alpha:.8});
			}
			if (autoOpen) Tweener.addTween(current, {time:(i*.07)+.5, onComplete:autoOpenMenu});
		}
